...
public class SmartDateUtil {
    ...
    public static LocalDate toLocalDate(final String text) {
        ...
        if (text.contains(DATE_SPLIT)) {
            return DateUtil.parseToLocalDate(string, "yyyy/MM/dd");
        }
        return DateUtil.parseToLocalDate(string, DateUtil.CHINESE_PATTERN);
    }
    ...
    public static Date toDate(final String text) {
        if (text.length() > DATE_LIMIT) {
            return DateUtil.localDateTimeToDate(toLocalDateTime(text));
        } else {
            return DateUtil.localDateToDate(toLocalDate(text));
        }
    }
    ...
    public static LocalDateTime toLocalDateTime(final String text) {
        ...
        // 2020-02-02 00:00:00
        return DateUtil.parseToLocalDateTime(string, DateUtil.FULL_CHINESE_PATTERN);
    }
}
